...
def main() -> None:
    ...
    # ---- offers ---------------------------------------------------------
    offers = [
        fake.offer(offer_id="ofr_COR_130", amount="130.00", title="BOG->COR morning flight"),
        fake.offer(offer_id="ofr_MDE_95", amount="95.00", title="BOG->MDE afternoon flight"),
        fake.offer(offer_id="ofr_MIA_300", amount="300.00", title="BOG->MIA direct"),
        fake.offer(
            offer_id="ofr_HTL_120",
            amount="120.00",
            category="hotels",
    ...
    write(
        "intent_130_approved.json",
        {
            **signed(
                fake.intent(
                    mandate_jti=claims.jti, offer_id="ofr_COR_130", amount="130.00", now=now
                )
            ),
    ...
    write(
        "intent_300_escalated.json",
        {
            **signed(
                fake.intent(
                    mandate_jti=claims.jti, offer_id="ofr_MIA_300", amount="300.00", now=now
                )
            ),
            "expect": {"decision": "ESCALATED", "reason_code": "AMOUNT_EXCEEDS_PER_TXN"},
        },
    )
    write(
        "intent_wrong_category.json",
        {
            **signed(
                fake.intent(
                    mandate_jti=claims.jti, offer_id="ofr_HTL_120", amount="120.00", now=now
                )
            ),
    ...
    write(
        "intent_wrong_key.json",
        {
            **signed(
            ...
            "expect": {"decision": "REJECTED", "reason_code": "INVALID_PROOF_OF_POSSESSION"},
    ...
    checkout = ap2.build_checkout_payload(
        ...
        line_items=[
            {
                "id": "ofr_COR_130",
                "label": "BOG->COR morning flight",
                "amount": ap2.to_minor_units("130.00"),
                "quantity": 1,
            }
        ],
        total_price="130.00",
