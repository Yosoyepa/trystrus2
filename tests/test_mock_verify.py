...
def test_in_mandate_purchase_is_approved(mandate, offer):
    intent = fake.intent(mandate_jti=mandate.jti, offer_id=offer.offer_id,
                         amount="130.00")
...
def test_the_mock_is_not_permissive(mandate, offer):
    ...
    over_limit = fake.intent(mandate_jti=mandate.jti, offer_id="ofr_MIA_300",
                             amount="300.00")
    wrong_category = fake.intent(mandate_jti=mandate.jti, offer_id="ofr_HTL_120",
                                 amount="120.00")

    assert verify(mandate, over_limit,
                  fake.offer(offer_id="ofr_MIA_300", amount="300.00")
                  ).decision is not DecisionOutcome.APPROVED
    assert verify(mandate, wrong_category,
                  fake.offer(offer_id="ofr_HTL_120", amount="120.00",
                             category="hotels")
                  ).decision is not DecisionOutcome.APPROVED
...
def test_non_active_mandate_is_refused_whatever_the_amount(
        mandate, offer, status, expected):
...
def test_revocation_wins_over_a_budget_problem(mandate, offer):
    ...
    decision = verify(mandate, intent, offer, spent="399",
...
def test_forbidden_category_is_rejected(mandate):
    ...
    intent = fake.intent(mandate_jti=mandate.jti, offer_id=hotel.offer_id,
                         amount="120.00")
...
def test_merchant_outside_the_allowlist_is_rejected(mandate, offer):
    intent = fake.intent(mandate_jti=mandate.jti, offer_id=offer.offer_id,
                         amount="130.00", merchant_id="not-vuelaya")
...
def test_expired_and_not_yet_valid_mandates(offer):
    ...
    expired = fake.mandate(valid_from=now - timedelta(days=10),
                           valid_until=now - timedelta(days=1))
    assert verify(expired, fake.intent(mandate_jti=expired.jti, **intent_kwargs),
                  offer).reason_code is ReasonCode.MANDATE_EXPIRED

    future = fake.mandate(valid_from=now + timedelta(days=1),
                          valid_until=now + timedelta(days=10))
    assert verify(future, fake.intent(mandate_jti=future.jti, **intent_kwargs),
                  offer).reason_code is ReasonCode.MANDATE_NOT_YET_VALID
...
def test_over_per_txn_limit_escalates_rather_than_refusing(mandate):
    ...
    intent = fake.intent(mandate_jti=mandate.jti, offer_id=big.offer_id,
                         amount="300.00")
    ...
    assert decision.diff == {"limit": "max_per_txn", "value": "150",
                             "attempted": "300.00"}
...
def test_canonical_fixture_states_the_same_threshold_twice():
    ...
    """
    canonical = fake.mandate(max_per_txn="150",
                             conditions={"<": [{"var": "offer.price"}, 150]})
    big = fake.offer(offer_id="ofr_MIA_300", amount="300.00")

    decision = verify(canonical,
                      fake.intent(mandate_jti=canonical.jti,
                                  offer_id=big.offer_id, amount="300.00"),
                      big)
...
def test_separated_ceiling_and_trigger_behave_sensibly():
    ...
    sane = fake.mandate(max_per_txn="200",
                        conditions={"<": [{"var": "offer.price"}, 150]})
    offer_170 = fake.offer(offer_id="ofr_X_170", amount="170.00")
    intent = fake.intent(mandate_jti=sane.jti, offer_id="ofr_X_170",
                         amount="170.00")
...
def test_budget_counts_reservations_not_just_spend(mandate, offer):
    ...
    assert verify(mandate, intent, offer, spent="200", reserved="0"
                  ).decision is DecisionOutcome.APPROVED
    assert verify(mandate, intent, offer, spent="200", reserved="130"
                  ).reason_code is ReasonCode.BUDGET_EXCEEDED
...
def test_budget_boundary_is_inclusive(mandate, offer):
    ...
    assert verify(mandate, intent, offer, spent="270"
                  ).decision is DecisionOutcome.APPROVED
    assert verify(mandate, intent, offer, spent="270.01"
                  ).reason_code is ReasonCode.BUDGET_EXCEEDED
...
def test_intent_amount_must_equal_the_catalog_price(mandate, offer):
    ...
    lying = fake.intent(mandate_jti=mandate.jti, offer_id=offer.offer_id,
                        amount="1.00")
...
def test_jsonlogic_condition_is_evaluated(mandate):
    ...
    assert verify(mandate, fake.intent(mandate_jti=mandate.jti,
                                       offer_id=cheap.offer_id, amount="95.00"),
                  cheap).decision is DecisionOutcome.APPROVED

    at_the_boundary = fake.offer(offer_id="ofr_X_150", amount="150.00")
    decision = verify(mandate, fake.intent(mandate_jti=mandate.jti,
                                           offer_id="ofr_X_150", amount="150.00"),
                      at_the_boundary)
...
def test_unsupported_jsonlogic_operator_raises_rather_than_passing():
    ...
    with pytest.raises(ValueError, match="unsupported JsonLogic operator"):
        verify(mandate, fake.intent(mandate_jti=mandate.jti,
                                    offer_id=offer.offer_id,
                                    amount=offer.amount), offer)
