...
from trustlib.jose import generate_ed25519, generate_p256, sign_compact
...
class StubIssuer:
    ...
    def issue(self, claims=None, **kwargs) -> tuple[str, str]:
        ...
        sd_jwt = sdjwt.issue(
            claims.model_dump(mode="json", exclude_none=True), self.issuer_key, kid=ISSUER_KID
        )
    ...
    def checkout(self, total: str = "130.00", currency: str = "USD") -> str:
        payload = ap2.build_checkout_payload(
            ...
            line_items=[
                {"id": "ofr_COR_130", "label": "BOG->COR", "amount": ap2.to_minor_units(total)}
            ],
            total_price=total,
            currency=currency,
        )
        return sign_compact(payload, self.merchant_key, kid=MERCHANT_KID, typ="JWT")
...
async def charge(
    ...
    *,
    ...
    return await capture(
        ...
        intent_ref=ids.new_id(ids.INTENT),
        purchase_id="pur_1",
...
async def test_capture_is_idempotent(session, verifier, token, issuer):
    ...
    first = await charge(
        session, verifier, token, issuer, key=key, sd_jwt=sd_jwt, checkout=checkout
    )
    await session.commit()
    second = await charge(
        session, verifier, token, issuer, key=key, sd_jwt=sd_jwt, checkout=checkout
...
async def test_concurrent_retries_charge_once(session, verifier, token, issuer, engine):
...
async def test_only_an_active_mandate_settles(session, verifier, token, issuer, status, expected):
    sd_jwt, jti = issuer.issue()
...
async def test_an_unreachable_issuer_refuses_rather_than_assumes(session, verifier, token, issuer):
    ...
    with pytest.raises(PaymentRefused) as refused:
        await charge(session, verifier, token, issuer, sd_jwt=sd_jwt, checkout=checkout)
...
async def test_a_mandate_from_another_issuer_is_refused(session, verifier, token, issuer):
    impostor = generate_ed25519()
    claims = ap2.apply_ap2_projection(fake.mandate())
    forged = sdjwt.issue(
        claims.model_dump(mode="json", exclude_none=True), impostor, kid=ISSUER_KID
    )
...
async def test_a_restated_cart_is_refused(session, verifier, token, issuer):
    ...
    with pytest.raises(PaymentRefused) as refused:
        await charge(
            session, verifier, token, issuer, amount="300.00", checkout=issuer.checkout("130.00")
        )

    assert refused.value.reason_code is ReasonCode.CONDITION_FAILED


async def test_a_checkout_signed_by_an_unknown_key_is_refused(session, verifier, token, issuer):
    stranger = generate_p256()
    payload = ap2.build_checkout_payload(
        order_id="ord_x",
        merchant_id="vuelaya",
        merchant_name="VuelaYa",
        merchant_website="https://x",
        line_items=[],
        total_price="130.00",
        currency="USD",
    )
...
# ==========================================================================
async def test_a_refusal_is_recorded_with_its_reason(session, verifier, token, issuer):
    ...
    rows = (
        (await session.execute(select(PaymentRow).where(PaymentRow.status == "refused")))
        .scalars()
        .all()
    )
...
# ==========================================================================
async def test_a_complete_evidence_chain_favours_the_seller(session, verifier, token, issuer):
    sd_jwt, jti = issuer.issue()
    checkout = issuer.checkout()
    settlement = await charge(session, verifier, token, issuer, sd_jwt=sd_jwt, checkout=checkout)
    await session.commit()

    dispute = await disputes.open_dispute(session, settlement.payment_id)
    resolved = await disputes.adjudicate(
        session,
        dispute.dispute_id,
        {
            "mandate_sd_jwt": sd_jwt,
...
async def test_a_missing_mandate_favours_the_buyer(session, verifier, token, issuer):
    ...
    dispute = await disputes.open_dispute(session, settlement.payment_id)
    resolved = await disputes.adjudicate(
        session, dispute.dispute_id, {"intent_jwt": "signed-intent"}
    )
...
async def test_evidence_for_a_different_cart_favours_the_buyer(session, verifier, token, issuer):
...
async def test_an_escalated_purchase_needs_its_approval_receipt(session, verifier, token, issuer):
    sd_jwt, _ = issuer.issue()
    checkout = issuer.checkout()
    settlement = await charge(session, verifier, token, issuer, sd_jwt=sd_jwt, checkout=checkout)
    await session.commit()
