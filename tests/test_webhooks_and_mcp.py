...
async def test_signed_yuno_webhook_is_accepted_and_tampering_is_401(tmp_path):
    ...
    try:
        async with AsyncClient(transport=ASGITransport(merchant_main.app),
                               base_url="http://merchant") as client:
            accepted = await client.post("/webhooks/yuno", content=body, headers=headers)
            rejected = await client.post(
                "/webhooks/yuno", content=body.replace(b"ynp_1", b"ynp_9"),
                headers=headers)
    finally:
...
async def test_successful_rail_capture_schedules_a_signed_captured_event(
    ...
    async def fake_capture(*_args, **_kwargs):
        return Settlement(
            payment_id="ynp_webhook", amount=Decimal("130.00"),
            currency="USD", mandate_jti="mdt_webhook",
    ...
    receipt = await yuno_main.create_payment(
        ...
        body=CaptureRequest(
            token_id="ynt_webhook", amount="130.00", intent_ref="int_webhook",
            purchase_id="pur_webhook", mandate_sd_jwt="signed", checkout_jwt="cart"),
...
async def test_mcp_has_exactly_three_tools_and_never_accepts_an_amount(session):
    ...
    assert {tool.name for tool in tools} == {
        "search_offers", "get_offer", "request_purchase"}
