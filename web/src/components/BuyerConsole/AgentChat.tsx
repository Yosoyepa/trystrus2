  ...
  const [messages, setMessages] = useState<AgentChatMessage[]>([
    {
      id: 'm1',
      sender: 'agent',
      text: "Hello Marta! I'm your autonomous flight booking agent (agt_flights). I have a verified purchase mandate for flights up to $150.00. Where would you like to travel?",
      ...
      if (purchaseRes.status.status === 'captured') {
        ...
        setMessages((prev) => [
          ...prev,
          {
            ...
            timestamp: new Date().toLocaleTimeString(),
            node: 'receipt',
            metadata: { receipt: purchaseRes.status.receipt },
          },
        ]);
      } else {
      ...
      if (purchaseRes.status.status === 'awaiting_escalation') {
        ...
        setMessages((prev) => [
          ...prev,
          {
            ...
            timestamp: new Date().toLocaleTimeString(),
            node: 'await_human',
            metadata: { escalation_id: purchaseRes.status.escalation_id },
          },
        ]);
      }
    ...
    <div className="rounded-2xl glass-panel p-5 sm:p-6 border border-slate-800/80 space-y-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              LLM Proposes · Deterministic Gate Disposes · No Payment Routes Outside Gate
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2">
        <input
          ...
          placeholder="Ask your agent to search or book flights..."
          disabled={isProcessing}
