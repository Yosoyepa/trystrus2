...
try:
    ...
    postgres_driver = pytest.importorskip(
        "psycopg2", reason="psycopg or psycopg2 is not installed"
    )
...
def test_velocity_counter_upsert_and_read(database_connection) -> None:
    ...
    try:
        store.increment_intent(
            mandate_id, Decimal("12.34"), now, transaction=database_connection
        )
        store.increment_intent(
            mandate_id, Decimal("7.66"), now, transaction=database_connection
        )
        ...
        with database_connection.cursor() as cursor:
            cursor.execute(
                "DELETE FROM velocity_counters WHERE mandate_id = %s", (mandate_id,)
            )
