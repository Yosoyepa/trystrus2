...
class ChargeService:
    ...
    async def charge(self, session: AsyncSession, body: ChargeRequest) -> Receipt:
        ...
        if offer is None:
            raise ChargeRefused(ReasonCode.CONDITION_FAILED,
                                "offer is not active in VuelaYa's catalogue")
        ...
        if body.intent.checkout_hash is None or not ap2.verify_checkout_binding(
                ...
                claimed_hash=body.intent.checkout_hash):
            raise ChargeRefused(ReasonCode.CONDITION_FAILED,
                                "intent is not bound to this Checkout JWT")
        if checkout.order.offer_id != offer.offer_id:
            raise ChargeRefused(ReasonCode.CONDITION_FAILED,
                                "Checkout JWT names another offer")
        if checkout.order.amount != offer.amount_decimal:
            raise ChargeRefused(ReasonCode.CONDITION_FAILED,
                                "quoted checkout no longer matches catalogue price")
        if checkout.order.status == "captured":
            ...
            try:
                ...
                raise ChargeSettlementError(
                    "stored Checkout JWT receipt is invalid") from exc
        if checkout.order.status != "quoted":
            raise ChargeRefused(ReasonCode.CONDITION_FAILED,
                                "Checkout JWT is not available for capture")
        ...
        try:
            await self._checkout.mark_captured(
                session, checkout=checkout, purchase_id=body.purchase_id,
                receipt=receipt,
            )
        except CheckoutJWTError as exc:
    ...
    async def _verify_mandate(self, token: str) -> MandateClaims:
        try:
            ...
            raise ChargeRefused(ReasonCode.INVALID_SIGNATURE,
                                "mandate SD-JWT does not verify") from exc

    def _verify_intent(self, body: ChargeRequest, claims: MandateClaims) -> None:
        intent = body.intent
        try:
            signed = verify_detached(body.intent_jwt, intent.model_dump(mode="json"),
                                     jwk_from_dict(claims.cnf.jwk))
            ...
            if not claims.payment_method_ref or \
                    body.payment_method_ref != claims.payment_method_ref:
                raise ValueError("payment token differs from the signed mandate")
        except Exception as exc:
            raise ChargeRefused(ReasonCode.INVALID_PROOF_OF_POSSESSION,
                                "agent detached JWS proof does not verify") from exc

    @staticmethod
    def _verify_price(body: ChargeRequest, offer: Offer) -> None:
        try:
            same = (body.intent.amount_decimal == offer.amount_decimal and
                    body.amount_decimal == offer.amount_decimal and
                    body.currency == offer.currency)
        except (InvalidOperation, ValueError) as exc:
            raise ChargeRefused(ReasonCode.CONDITION_FAILED,
                                "amount is not a valid merchant price") from exc
        if not same:
            raise ChargeRefused(ReasonCode.CONDITION_FAILED,
                                "intent amount must equal the current offer price")
