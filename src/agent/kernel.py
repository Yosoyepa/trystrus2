...
def gate(*, claims: dict, intent: dict, offer: dict, spend: dict, now: int) -> Decision:
    ...
    if scope.get("categories") and offer["category"] not in scope["categories"]:
        return Decision(REJECTED, "CATEGORY_FORBIDDEN",
                        f"{offer['category']} is not in {scope['categories']}")
    if scope.get("merchants") and offer["merchant_id"] not in scope["merchants"]:
        return Decision(REJECTED, "MERCHANT_NOT_ALLOWED",
                        f"{offer['merchant_id']} is not in {scope['merchants']}")

    # currency
    if intent["currency"] != claims.get("currency", "USD"):
        return Decision(REJECTED, "CONDITION_FAILED",
    ...
    if max_txn.get("count") is not None and spend["txn_count"] >= int(max_txn["count"]):
        return Decision(REJECTED, "LIMIT_EXHAUSTED",
    ...
    if amount > remaining:
        return Decision(REJECTED, "BUDGET_EXCEEDED",
    ...
    if amount > dec(limits["max_per_txn"]):
        return Decision(ESCALATED, "AMOUNT_EXCEEDS_PER_TXN",
                        f"{fmt(amount)} is over the {limits['max_per_txn']} per-purchase limit",
                        {"amount": fmt(amount), "limit": limits["max_per_txn"],
...
def verify(conn, *, mandate_token: str, intent: dict, intent_sig: str,
    ...
    if intent.get("mandate_jti") != claims["jti"]:
        return _refuse(conn, claims["jti"], "INVALID_PROOF_OF_POSSESSION",
                       "intent does not name this mandate")

    # freshness and replay (C6, C7)
    now = now_ts()
    if int(intent["exp"]) - int(intent["iat"]) > INTENT_TTL_S:
        return _refuse(conn, claims["jti"], "INVALID_SIGNATURE",
                       f"intent lifetime over {INTENT_TTL_S}s")
    if now > int(intent["exp"]):
        return _refuse(conn, claims["jti"], "INVALID_SIGNATURE", "intent expired")
    seen = conn.execute(
        "SELECT 1 FROM purchase_intents WHERE jti=? OR nonce=?",
        (intent["jti"], intent["nonce"])).fetchone()
    if seen:
    ...
    for ancestor in chain(conn, claims["jti"])[1:]:
        arow = conn.execute("SELECT status FROM mandates WHERE jti=?",
                            (ancestor,)).fetchone()
        if arow and arow["status"] != "active":
            return _refuse(conn, claims["jti"], "MANDATE_REVOKED",
    ...
    if fmt(intent["amount"]) != fmt(offer["price"]):
        return _refuse(conn, claims["jti"], "AMOUNT_MISMATCH",
                       f"intent {intent['amount']} != catalog {offer['price']}")

    spend = {"spent_total": row["spent_total"], "reserved_amount": row["reserved_amount"],
    ...
    if decision.decision != APPROVED or not reserve:
        audit.append(conn, "purchase.gated",
                     {"intent_jti": intent["jti"], **decision.as_dict()},
                     agent_id=claims["agent"], mandate_jti=claims["jti"])
    ...
    if not reserve_chain(conn, claims["jti"], intent["amount"]):
        return _refuse(conn, claims["jti"], "BUDGET_EXCEEDED",
                       "mandate state changed during the check, or an ancestor "
                       "mandate cannot cover it")
    ...
    conn.execute(
        ...
        (intent["jti"], claims["jti"], claims["agent"], intent["nonce"],
         json.dumps(intent), intent_sig, now_iso()))
    audit.append(conn, "purchase.verified",
                 {"intent_jti": intent["jti"], "reservation_id": reservation_id,
                  "amount": intent["amount"], "offer_id": intent["offer_id"]},
                 agent_id=claims["agent"], mandate_jti=claims["jti"])
    return {"decision": APPROVED, "reason_code": None, "detail": "",
            "diff": {}, "reservation_id": reservation_id, "mandate_jti": claims["jti"]}


def _refuse(conn, mandate_jti: str | None, code: str, detail: str) -> dict[str, Any]:
    audit.append(conn, "purchase.refused", {"reason_code": code, "detail": detail},
...
def chain(conn, mandate_jti: str) -> list[str]:
    ...
    while current and current not in seen:
        seen.append(current)
        row = conn.execute("SELECT parent_jti FROM mandates WHERE jti=?",
                           (current,)).fetchone()
...
def reserve_chain(conn, mandate_jti: str, amount: str) -> bool:
    ...
    for jti in chain(conn, mandate_jti):
        ...
        cur = conn.execute(
            ...
            "AND txn_count=?",
            (fmt(dec(row["reserved_amount"]) + dec(amount)), now_iso(), jti,
             row["reserved_amount"], row["spent_total"], row["txn_count"]))
        if cur.rowcount == 0:
            _unreserve(conn, done, amount)
            return False
        done.append(jti)
...
def _unreserve(conn, jtis: list[str], amount: str) -> None:
    for jti in jtis:
        row = conn.execute("SELECT reserved_amount FROM mandates WHERE jti=?",
                           (jti,)).fetchone()
        if row:
            freed = max(Decimal("0"), dec(row["reserved_amount"]) - dec(amount))
            conn.execute("UPDATE mandates SET reserved_amount=? WHERE jti=?",
                         (fmt(freed), jti))
...
def settle(conn, mandate_jti: str, amount: str) -> None:
    ...
    for jti in chain(conn, mandate_jti):
        ...
        conn.execute(
            ...
            (fmt(freed), fmt(spent), count,
             "exhausted" if exhausted else row["status"], now_iso(), jti))
...
def submit_purchase(conn, *, offer_id: str, mandate_jti: str,
    ...
    signature = jws.sign_detached(intent, agent_private_key(claims["agent"]),
                                  kid=claims["agent"], typ="purchase-intent+jws")
    ...
    if decision["decision"] == ESCALATED:
        conn.execute("UPDATE purchases SET status='awaiting_escalation', reason_code=?,"
                     " updated_at=? WHERE id=?",
                     (decision["reason_code"], now_iso(), purchase_id))
        from .escalation import open_escalation
        esc = open_escalation(conn, purchase_id=purchase_id, mandate_jti=mandate_jti,
                              run_id=run_id, diff=decision.get("diff", {}),
                              reason_code=decision["reason_code"])
        return {"status": "escalated", "purchase_id": purchase_id,
                "escalation_id": esc["id"], "reason_code": decision["reason_code"],
                "detail": decision["detail"], "diff": decision.get("diff", {}),
                "intent": intent, "signature": signature}

    if decision["decision"] != APPROVED:
        conn.execute("UPDATE purchases SET status='rejected', reason_code=?, updated_at=?"
                     " WHERE id=?", (decision["reason_code"], now_iso(), purchase_id))
        audit.append(conn, "purchase.rejected",
                     {"purchase_id": purchase_id, "reason_code": decision["reason_code"],
                      "detail": decision["detail"]},
                     agent_id=claims["agent"], run_id=run_id, mandate_jti=mandate_jti)
        return {"status": "rejected", "purchase_id": purchase_id,
                "reason_code": decision["reason_code"], "detail": decision["detail"]}

    return _charge(conn, purchase_id=purchase_id, mandate_row=mandate_row, claims=claims,
                   intent=intent, signature=signature, decision=decision, run_id=run_id)
...
def _charge(conn, *, purchase_id, mandate_row, claims, intent, signature, decision,
    ...
    if not result.get("accepted"):
        return _compensate(conn, purchase_id, claims, intent,
                           ...
                           result.get("detail", ""), run_id)

    settle(conn, claims["jti"], intent["amount"])
    conn.execute("UPDATE purchases SET status='captured', receipt=?, updated_at=?"
                 " WHERE id=?", (json.dumps(result["receipt"]), now_iso(), purchase_id))
    conn.execute("UPDATE purchase_intents SET status='captured' WHERE jti=?",
                 (intent["jti"],))
    audit.append(conn, "purchase.captured",
                 {"purchase_id": purchase_id, "receipt": result["receipt"]},
                 agent_id=claims["agent"], run_id=run_id, mandate_jti=claims["jti"])
    return {"status": "captured", "purchase_id": purchase_id,
            "receipt": result["receipt"]}


def _compensate(conn, purchase_id, claims, intent, code, detail, run_id) -> dict:
    release(conn, claims["jti"], intent["amount"])
    conn.execute("UPDATE purchases SET status='compensated', reason_code=?, updated_at=?"
                 " WHERE id=?", (code, now_iso(), purchase_id))
    audit.append(conn, "purchase.compensated",
                 {"purchase_id": purchase_id, "reason_code": code, "detail": detail},
                 agent_id=claims["agent"], run_id=run_id, mandate_jti=claims["jti"])
    return {"status": "rejected", "purchase_id": purchase_id, "reason_code": code,
            "detail": detail}
