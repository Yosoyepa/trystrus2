...
PERMISSIONS: dict[str, set[str]] = {
    "admin":   {"agent.create", "agent.publish", "agent.people", "agent.status",
                "watch.create", "watch.cancel", "escalation.resolve",
                "mandate.revoke", "read"},
    ...
    "owner":   {"agent.create", "agent.publish", "agent.people", "agent.status",
                "watch.create", "watch.cancel", "mandate.revoke",
                "escalation.resolve", "read"},
...
AGENT_SCOPED = {"agent.publish", "agent.people", "agent.status", "watch.create",
                "watch.cancel", "escalation.resolve"}
...
def issue_token(conn, person_id: str) -> str:
    ...
    updated = conn.execute("UPDATE people SET token_hash=? WHERE id=?",
                           (hash_token(token), person_id)).rowcount
    if not updated:
        raise AuthError("NO_SUCH_PERSON", person_id)
    audit.append(conn, "person.token.issued", {"person_id": person_id},
                 actor=person_id)
    return token
...
def authenticate(conn, token: str | None) -> Principal:
    ...
    if not token:
        raise AuthError("NO_TOKEN",
                        "set TT_TOKEN, or pass --token; the console is not anonymous")
    row = conn.execute(
        "SELECT id, name, role FROM people WHERE token_hash=?",
...
def authorize(conn, principal: Principal, action: str,
              agent_id: str | None = None) -> None:
    allowed = PERMISSIONS.get(principal.role, set())
    if action not in allowed:
        raise AuthError("FORBIDDEN",
                        f"{principal} may not {action}")
    if action in AGENT_SCOPED and agent_id and principal.role != "admin":
        row = conn.execute(
            "SELECT owner_id, approver_id, auditor_id FROM agents WHERE id=?",
            (agent_id,)).fetchone()
        if row is None:
        ...
        if principal.person_id not in attached:
            raise AuthError("FORBIDDEN",
                            f"{principal} is not attached to {agent_id}")
        if action == "escalation.resolve" and principal.person_id != row["approver_id"]:
            raise AuthError("FORBIDDEN",
                            f"{principal} is not the approver for {agent_id}")
        if action != "escalation.resolve" and principal.person_id != row["owner_id"]:
            raise AuthError("FORBIDDEN",
                            f"{principal} does not own {agent_id}")


def require(conn, token: str | None, action: str,
            agent_id: str | None = None) -> Principal:
    ...
    try:
        ...
        audit.append(conn, "auth.denied",
                     {"person_id": principal.person_id, "action": action,
                      "agent_id": agent_id, "reason": exc.code},
                     actor=principal.person_id, agent_id=agent_id)
        raise
    return principal
