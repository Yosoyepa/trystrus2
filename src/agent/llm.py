...
def parse_request(text: str) -> dict[str, Any]:
    try:
        parsed = _json(complete(PARSE_SYSTEM, fence("buyer_request", text),
                                max_tokens=120))
...
def propose(*, request: str, offers: list[dict], ontology_text: str,
            memory_text: str, guidance: str = "") -> dict[str, Any]:
    """Pick one offer. Returns {offer_id, why, concern, source}."""
    if not offers:
        return {"offer_id": None, "why": "nothing in the catalog matches",
                "concern": None, "source": "deterministic"}
    catalog = "\n".join(
    ...
    try:
        ...
        if any(o["offer_id"] == chosen for o in offers):
            return {"offer_id": chosen, "why": str(answer.get("why", ""))[:280],
                    "concern": answer.get("concern"), "source": LLM_MODEL}
...
def _propose_fallback(offers: list[dict]) -> dict[str, Any]:
    ...
    return {"offer_id": cheapest["offer_id"],
