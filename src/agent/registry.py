...
# ── people ───────────────────────────────────────────────────────────────────
def add_person(conn, name: str, email: str | None = None, role: str = "member",
    ...
    audit.append(conn, "person.created", {"person_id": pid, "name": name, "role": role},
                 actor=pid)
    return pid
...
# ── agents ───────────────────────────────────────────────────────────────────
def create_agent(conn, name: str, *, owner_id: str, approver_id: str | None = None,
                 auditor_id: str | None = None, ontology: dict | None = None,
                 model_cfg: dict | None = None, actor: str | None = None,
    ...
    conn.execute(
        ...
        (aid, name, owner_id, approver_id or owner_id, auditor_id or owner_id,
         json.dumps(jwk), stamp, stamp),
    )
    audit.append(conn, "agent.created",
                 {"agent_id": aid, "name": name, "owner_id": owner_id,
                  "approver_id": approver_id or owner_id, "public_jwk": jwk},
                 actor=actor or owner_id, agent_id=aid)
    publish_version(conn, aid, ontology or {}, model_cfg or {},
                    changed_by=actor or owner_id, reason="initial version")
    return aid


def publish_version(conn, agent_id: str, ontology: dict, model_cfg: dict | None = None,
                    *, changed_by: str | None = None, reason: str = "") -> int:
    row = conn.execute("SELECT current_version FROM agents WHERE id=?",
                       (agent_id,)).fetchone()
    if row is None:
    ...
    try:
        conn.execute(
            ...
            (agent_id, version, json.dumps(ontology), json.dumps(model_cfg or {}),
             changed_by, reason, stamp),
        )
        conn.execute("UPDATE agents SET current_version=?, updated_at=? WHERE id=?",
                     (version, stamp, agent_id))
        conn.execute("COMMIT")
    except Exception:
        conn.execute("ROLLBACK")
        raise
    audit.append(conn, "agent.version.published",
                 {"agent_id": agent_id, "version": version, "reason": reason,
                  "ontology_keys": sorted(ontology.keys())},
                 actor=changed_by, agent_id=agent_id)
    return version
...
def history(conn, agent_id: str) -> list[dict]:
    return conn.execute(
        ...
        "ORDER BY v.version DESC", (agent_id,)).fetchall()


def set_people(conn, agent_id: str, *, owner_id=None, approver_id=None,
               auditor_id=None, actor: str | None = None) -> None:
    ...
    audit.append(conn, "agent.people.changed", {"agent_id": agent_id, **new},
...
def set_status(conn, agent_id: str, status: str, actor: str | None = None) -> None:
    ...
    conn.execute("UPDATE agents SET status=?, updated_at=? WHERE id=?",
                 (status, now_iso(), agent_id))
    audit.append(conn, "agent.status.changed", {"agent_id": agent_id, "status": status},
                 actor=actor, agent_id=agent_id)
