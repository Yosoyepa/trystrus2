...
def capture(conn, *, token_ref: str, amount: str, currency: str,
    ...
    """
    prior = conn.execute(
        "SELECT response FROM idempotency_keys WHERE key=? AND scope='rail.capture'",
        (request_id,)).fetchone()
    if prior:
        return {**json.loads(prior["response"]), "idempotent_replay": True}

    row = conn.execute("SELECT * FROM payment_instruments WHERE token_ref=?",
                       (token_ref,)).fetchone()
    if row is None:
        raise RailError("RAIL_ERROR", f"unknown payment token {token_ref}")
    if row["status"] != "active":
        raise RailError("RAIL_TOKEN_DELETED",
                        "payment token was deleted at the rail (revocation)")
    ...
    result = {
        "capture_id": new_id("cap"), "status": "COMPLETED",
        "amount": fmt(amount), "currency": currency, "token_ref": token_ref,
        "captured_at": now_iso(),
    }
    conn.execute(
...
def open_dispute(conn, capture_id: str, reason: str = "UNAUTHORISED") -> dict[str, Any]:
    ...
    return {"dispute_id": new_id("dsp"), "capture_id": capture_id,
            "reason": reason, "status": "OPEN", "opened_at": now_iso()}
