...
from . import audit, escalation, graph, llm, registry
from .ids import new_id, now_iso

APPROVE_WORDS = {"approve", "approved", "yes", "y", "ok", "okay", "go", "go ahead",
                 "do it", "buy it", "confirm", "confirmed", "si", "sí", "dale",
                 "aprobar", "apruebo", "adelante"}
REJECT_WORDS = {"reject", "rejected", "no", "n", "cancel", "stop", "deny", "denied",
                "nope", "rechazar", "rechazo", "para", "detente"}
...
def _log(conn, session_id: str, role: str, text: str, run_id: str | None = None) -> None:
    conn.execute(
        ...
        " VALUES(?,?,?,?,?)", (session_id, role, text, run_id, now_iso()))
...
class Session:
    ...
    def __init__(self, conn, *, agent_id: str, mandate_jti: str,
                 session_id: str | None = None, person: str = "buyer"):
        self.conn = conn
    ...
    def send(self, text: str) -> list[str]:
        ...
        for line in replies:
            _log(self.conn, self.session_id, "agent", line,
                 run_id=(self.active_run() or {}).get("run_id"))
        return replies

    # ── turn shapes ──────────────────────────────────────────────────────────
    def _new_request(self, text: str) -> list[str]:
        run = graph.start(self.conn, agent_id=self.agent_id,
                          mandate_jti=self.mandate_jti, request=text,
    ...
    def _answer_escalation(self, run: dict, text: str) -> list[str]:
        ...
        if escalation.is_expired(self.conn, esc_id):
            ...
            return ["That request timed out, so I refused it. Nothing was charged.",
                    "Silence never approves a purchase here."]

        intent = classify(text)
        if intent == "approve":
            result = escalation.resolve(self.conn, esc_id, decision="APPROVE",
                                        approver=self.person, channel="chat",
                                        sticky=True)
            run = graph.resume(self.conn, run["run_id"])
            outcome = result.get("outcome") or {}
            lines = ["Approved. Re-running the check before paying — an approval "
                     "authorises a retry, not a bypass."]
            return lines + self._describe(run, outcome)

        if intent == "reject":
            escalation.resolve(self.conn, esc_id, decision="REJECT",
                               approver=self.person, channel="chat")
            graph.resume(self.conn, run["run_id"])
        ...
        escalation.resolve(self.conn, esc_id, decision="REJECT",
                           approver=self.person, channel="chat")
        graph.resume(self.conn, run["run_id"])
        fresh = graph.start(self.conn, agent_id=self.agent_id,
    ...
    def _describe(self, run: dict, outcome: dict | None = None) -> list[str]:
        ...
        if run["status"] == "awaiting_human":
            ...
            if diff.get("over_by"):
                lines.append(f"It is {diff['over_by']} over your per-purchase limit "
                             f"of {diff.get('limit')}.")
            lines.append("Reply 'approve' to allow it, 'reject' to refuse, or tell me "
        ...
        if status == "captured":
            ...
            lines.append(f"Bought: {receipt.get('title')} for "
                         f"{receipt.get('amount')} {receipt.get('currency')}.")
            lines.append(f"Under mandate {receipt.get('mandate_jti')} · "
        ...
        if status in ("rejected", "compensated"):
            lines.append(f"Refused: {result.get('reason_code')} — "
    ...
    def _offer_line(self, state: dict, proposal: dict) -> str:
        for offer in state.get("offers", []):
            if offer["offer_id"] == proposal.get("offer_id"):
                return (f"{offer['title']} at {offer['price']} {offer['currency']} "
                        f"({offer['offer_id']})")
