...
def open_escalation(
    conn, *, purchase_id: str, mandate_jti: str, run_id: str | None, diff: dict, reason_code: str
) -> dict[str, Any]:
    ...
    esc_id = new_id("esc")
    timeout_at = (
        (_dt.datetime.now(_dt.UTC) + _dt.timedelta(seconds=ESCALATION_TIMEOUT_S))
    ...
    conn.execute(
        ...
        (
            ...
            run_id,
            approver,
            json.dumps({**diff, "reason_code": reason_code}),
            timeout_at,
            now_iso(),
        ),
    )
...
def resolve(
    conn, esc_id: str, *, decision: str, approver: str, channel: str = "chat", sticky: bool = False
) -> dict[str, Any]:
    ...
    receipt_sig = jws.sign_detached(
        receipt, load_or_create("issuer"), kid="v1", typ="approval-receipt+jws"
    )
    conn.execute(
...
def expire(conn, esc_id: str) -> dict[str, Any]:
    ...
    conn.execute(
        "UPDATE escalations SET status='expired', decision='TIMEOUT' WHERE id=?", (esc_id,)
    )
    conn.execute(
        "UPDATE purchases SET status='rejected', reason_code=?, updated_at=? WHERE id=?",
...
def _retry_through_gate(conn, row, approver: str, *, sticky: bool) -> dict[str, Any]:
    ...
    purchase = conn.execute("SELECT * FROM purchases WHERE id=?", (row["purchase_id"],)).fetchone()
    intent_row = conn.execute(
        "SELECT * FROM purchase_intents WHERE jti=?", (purchase["intent_jti"],)
    ).fetchone()
    ...
    if sticky:
        ...
        derived = mandate_mod.issue(
            ...
            validity=claims["validity"],
            currency=claims["currency"],
            parent_jti=mandate_jti,
            signed_with=f"approval:{approver}",
        )
        mandate_jti = derived["jti"]

    return kernel.submit_purchase(
        conn,
        offer_id=offer_id,
        mandate_jti=mandate_jti,
...
def sweep(conn) -> list[dict[str, Any]]:
    ...
    rows = conn.execute(
        "SELECT id FROM escalations WHERE status='pending' AND timeout_at < ?", (now_iso(),)
    ).fetchall()
