...
def append(
    ...
    *,
    ...
    row = {
        ...
        "created_at": created_at, "chain_key": key,
    }
    conn.execute("BEGIN")
    try:
        ...
        head = conn.execute(
            "SELECT head_hash, length FROM chains WHERE chain_key=? FOR UPDATE",
            (key,)).fetchone()
        ...
        conn.execute(
            ...
            "VALUES(?,?,?,?,?,?,?,?,?,?,?,?)",
            (key, chain_seq, event_id, type, actor, agent_id, run_id, mandate_jti,
    ...
    return {"event_id": event_id, "hash": digest, "prev_hash": prev_hash,
            "chain_key": key, "chain_seq": chain_seq}
...
def verify_chain(conn, chain_key: str) -> dict[str, Any]:
    ...
    for row in conn.execute(
            "SELECT * FROM audit_events WHERE chain_key=? ORDER BY chain_seq ASC",
            (chain_key,)):
        rebuilt = {
        ...
        if row["prev_hash"] != prev_hash:
            return {"valid": False, "chain_key": chain_key, "checked": checked,
                    "seq": row["seq"], "chain_seq": row["chain_seq"],
        ...
        prev_hash = row["hash"]
        checked += 1
    stored = conn.execute("SELECT head_hash FROM chains WHERE chain_key=?",
                          (chain_key,)).fetchone()
...
def verify_all(conn) -> dict[str, Any]:
    ...
    results = [verify_chain(conn, row["chain_key"])
...
def compute_root(conn) -> str:
    ...
    heads = [(r["chain_key"], r["head_hash"]) for r in
             conn.execute("SELECT chain_key, head_hash FROM chains ORDER BY chain_key")]
    return hashlib.sha256(canonical_json(
        [{"chain_key": k, "head": h} for k, h in heads]).encode("utf-8")).hexdigest()
...
def checkpoint(conn) -> dict[str, Any]:
    ...
    rows = conn.execute(
        "SELECT chain_key, head_hash, length FROM chains ORDER BY chain_key").fetchall()
    ...
    signature = b64u(load_or_create("evidence_root").sign(
...
def verify_checkpoint(conn) -> dict[str, Any]:
    ...
    if not replay["valid"]:
        ...
        return {"valid": False, "signed_root": row["root_hash"],
                "current_root": current, "at": row["created_at"],
                "detail": f"chain {broken['chain_key']} no longer replays: "
                          f"{broken['error']}"}
    if row["root_hash"] != current:
        return {"valid": False, "signed_root": row["root_hash"],
                "current_root": current, "at": row["created_at"],
                "detail": "chains changed since this checkpoint "
                          "(new events, or a rewritten head)"}
    return {"valid": True, "signed_root": row["root_hash"], "current_root": current,
            "at": row["created_at"], "detail": ""}
